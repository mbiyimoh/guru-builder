...
- **Test Project**: E2E Test Project (ID: cmg30m2ll00001cs8vu82357k)
- **Prerequisites**:
  - Dev server running: `PORT=3009 npm run dev`
  - Test data seeded: `npm run db:seed:test` (runs automatically with test:e2e)
  - Environment variables configured (E2E_TEST_MODE, OPENAI_API_KEY, SUPABASE_URL)
  - See `tests/README.md` for complete E2E testing setup documentation
...
- **Run command**: `npm run test:e2e` or `npx playwright test <feature-name>`
- **Environment**: Real browser (Chromium - no OAuth needed with E2E_TEST_MODE)
- **Speed**: Fast (~30 seconds - 2 minutes with authentication bypass)
- **Use for**: [List 2-3 UI flows to test]
- **Authentication**: Automatic bypass via E2E_TEST_MODE (test user: test@example.com)
...
This project uses `E2E_TEST_MODE` to bypass Supabase OAuth during testing:
- Test data automatically seeded before each run (user + project)
- No manual authentication required
- Tests use mock user (test@example.com) and test project (cmg30m2ll00001cs8vu82357k)
- All protected routes accessible without OAuth flow
- See `tests/README.md` and `lib/e2e-utils.ts` for implementation details
