 ...
 * Usage: node scripts/run-self-play-direct.js [gamesCount]
 */
...
async function main() {
  ...
  try {
    ...
    const selfPlayModule = await import('../lib/positionLibrary/selfPlayGenerator.js');
    const { runSelfPlayBatch } = selfPlayModule;
    ...
    if (newPositions.length > 0) {
      ...
      const byPhase = { OPENING: 0, EARLY: 0, MIDDLE: 0, BEAROFF: 0 };
      const BATCH_SIZE = 50;
      let stored = 0;
