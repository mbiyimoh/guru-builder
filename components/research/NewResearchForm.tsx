...
export function NewResearchForm({ projectId, projectName }: NewResearchFormProps) {
  ...
  return (
    <div className="bg-white rounded-lg border">
      <form onSubmit={handleSubmit} className="p-6">
        <div className="space-y-6">
          ...
          <div>
            ...
            <button
              type="button"
              onClick={() => setShowPromptEngineer(true)}
              disabled={!instructions.trim()}
              className="mt-3 inline-flex items-center px-3 py-1.5 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
            >
      ...
      {showPromptEngineer && (
        <PromptEngineerModal
          roughInstructions={instructions}
          onClose={() => setShowPromptEngineer(false)}
          onApply={(refinedPrompt) => setInstructions(refinedPrompt)}
        />
      )}
    </div>
  );
